...
export default function PdfToHtmlPage() {
  useSEO({
    title: "PDF to HTML Free Online | PDF HUB 24",
    description: "Convert PDF documents to HTML web pages free online. Preserve text, links and layout. No signup. PDF HUB 24.",
    keywords: "pdf-to-html free, pdf-to-html online, pdf-to-html no signup, pdf hub 24",
  ...
  const handleProcess = async () => {
    ...
    try {
      await runWithProgress(async () => {
        ...
        const res = await fetch("/api/pdf-to-html", { method: "POST", body: formData });
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 py-8">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="mb-8">
            <h1 className="text-3xl md:text-4xl font-bold mb-3">PDF to HTML</h1>
            <p className="text-muted-foreground leading-relaxed mb-4">Convert PDF documents to HTML web pages free online. Preserve text, links and layout. No signup. PDF HUB 24.</p>
            <TrustBadges />
          </div>
          <div className="space-y-6">
            ...
            {files.length > 0 && status === "idle" && (
              <div className="rounded-lg border bg-card p-6 space-y-4">
                ...
                <Button onClick={handleProcess} className="w-full" size="lg">PDF to HTML</Button>
              </div>
            )}
            <ProcessingState status={status === "processing" || status === "error" ? status : "idle"} progress={progress} message="Converting PDF to HTML..." />
            {status === "success" && resultUrl && (
              <div className="rounded-lg border bg-card p-6 space-y-4">
                <h3 className="font-semibold">Done! Your file is ready.</h3>
                <Button onClick={() => { const a = document.createElement("a"); a.href = resultUrl!; a.download = (files[0]?.name?.replace(/\.pdf$/i, "") || "file") + ".html"; a.click(); }} className="w-full" size="lg"><Download className="w-4 h-4 mr-2" />Download File</Button>
          ...
          <RelatedTools currentToolId="pdf-to-html" />
        </div>
      </main>
      <Footer />
    </div>
