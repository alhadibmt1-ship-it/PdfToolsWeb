...
export default function WhiteoutPdfPage() {
  useSEO({
    title: "Whiteout PDF Free Online | PDF HUB 24",
    description: "Cover and hide content in PDF with white boxes free online. Block out text and images easily. No signup. PDF HUB 24.",
    keywords: "whiteout-pdf free, whiteout-pdf online, whiteout-pdf no signup, pdf hub 24",
  ...
  const handleProcess = async () => {
    ...
    try {
      await runWithProgress(async () => {
        ...
        const res = await fetch("/api/whiteout-pdf", { method: "POST", body: formData });
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 py-8">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="mb-8">
            <h1 className="text-3xl md:text-4xl font-bold mb-3">Whiteout PDF</h1>
            <p className="text-muted-foreground leading-relaxed mb-4">Cover and hide content in PDF with white boxes free online. Block out text and images easily. No signup. PDF HUB 24.</p>
            <TrustBadges />
          </div>
          <div className="space-y-6">
            ...
            {files.length > 0 && status === "idle" && (
              <div className="rounded-lg border bg-card p-6 space-y-4">
                ...
                <Button onClick={handleProcess} className="w-full" size="lg">Whiteout PDF</Button>
              </div>
            )}
            <ProcessingState status={status === "processing" || status === "error" ? status : "idle"} progress={progress} message="Processing PDF..." />
            {status === "success" && resultUrl && (
              <div className="rounded-lg border bg-card p-6 space-y-4">
                ...
                <div className="pt-3 border-t border-border/50 flex flex-wrap justify-center gap-x-4 gap-y-1 text-sm">
                  ...
                  <Link href="/redact-pdf" className="text-primary hover:underline font-medium" data-testid="link-post-download-1">Redact PDF →</Link>
                  <Link href="/flatten-pdf" className="text-primary hover:underline font-medium" data-testid="link-post-download-2">Flatten PDF →</Link>
                </div>
              </div>
            )}
          </div>
          <RelatedTools currentToolId="whiteout-pdf" />
        </div>
      </main>
      <Footer />
    </div>
